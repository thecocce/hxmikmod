...
class ITPACK {
        public var bits:UWORD;    /* current number of bits */
        public var bufbits:UWORD; /* bits in buffer */
        public var last:SWORD;    /* last output */
        public var buf:UBYTE;     /* bit buffer */
...
class SLoader {
   ...
   static var musiclist:Array<SAMPLOAD>;
   static var sndfxlist:Array<SAMPLOAD>;
   static  var sl_rlength:Int;
   static  var sl_old:SWORD;
   static  var sl_buffer:Array<SWORD>;
   ...
   public static function SL_Exit(s:SAMPLOAD) {
        if(sl_rlength>0) MMio._mm_fseek(s.reader,sl_rlength,MMio.SEEK_CUR);
        if(sl_buffer!=null) {
                sl_buffer=null;
        }
   }

   static function FreeSampleList(s:Array<SAMPLOAD>) {
	var i:Int;
	for (i in 0 ... s.length) s[i]=null;
   }


   static function RealSpeed(s:SAMPLOAD):ULONG {
        return Std.int(s.sample.speed/(s.scalefactor!=0?s.scalefactor:1));
   }    


   static function SL_Sample16to8(s:SAMPLOAD) {
        s.outfmt &= ~Defs.SF_16BITS;
        s.sample.flags = (s.sample.flags&~Defs.SF_FORMATMASK) | s.outfmt;
   }

   public static function SL_Sample8to16(s:SAMPLOAD) {
        s.outfmt |= Defs.SF_16BITS;
        s.sample.flags = (s.sample.flags&~Defs.SF_FORMATMASK) | s.outfmt;
   }

   public static function SL_SampleSigned(s:SAMPLOAD) {
        s.outfmt |= Defs.SF_SIGNED;
        s.sample.flags = (s.sample.flags&~Defs.SF_FORMATMASK) | s.outfmt;
   }

   static function SL_SampleUnsigned(s:SAMPLOAD) {
        s.outfmt &= ~Defs.SF_SIGNED;
        s.sample.flags = (s.sample.flags&~Defs.SF_FORMATMASK) | s.outfmt;
   }
   ...
   public static function SL_LoadSamples():Int {
	...
	switch(stage_LoadSamples) {
	  case 0:
		if((musiclist==null)&&(sndfxlist==null)) return 1;
   ...
   static function SL_HalveSample(s:SAMPLOAD,factor:Int) {
   ...
   static function SampleTotal(samplist:Array<SAMPLOAD>,type:Int):ULONG {
        var total = 0;
	var i:Int;
   ...
   // cast from UWORD to SWORD
   inline static function sword(x:UWORD) {
	x&=0xffff;
	if (x>=32768) x-=65536;
	return x;
   }

   // cast from UWORD to SBYTE
   inline static function sbyte(x:UWORD) {
	x&=0xff;
	if (x>=128) x-=256;
	return x;
   }
   ...
   static function read_itcompr8(status:ITPACK,reader:MREADER,sl_buffer:Array<SWORD>,count:UWORD,incnt:Array<UWORD>):Int {
	...
	var new_count=false;
        ...
        while (desti<count) {
                ...
                while (needbits!=0) {
                        ...
                        if (bufbits==0) {
                                if(incnt[0]--!=0)
                                        buf=MMio._mm_read_UBYTE(reader);
                                else
                                        buf=0;
                                bufbits=8;
                        }
			...
			trace("it compr8 err desti="+desti+" bits="+bits+" bufbits="+bufbits);
                        MMio._mm_errno=Defs.MMERR_ITPACK_INVALID_DATA;
                        return 0;
                }
   ...
   static function read_itcompr16(status:ITPACK,reader:MREADER,sl_buffer:Array<SWORD>,count:UWORD,incnt:Array<UWORD>):Int {
        //SWORD *dest=sl_buffer,*end=sl_buffer+count;
	var desti=0;
	var x:SLONG;
	var y:SLONG;
	...
	var buf=status.buf;
        ...
        while (desti<count) {
                ...
                while (needbits!=0) {
                        ...
                        /* get as many bits as necessary */
                        y=needbits<bufbits?needbits:bufbits;
			if (y<=0) { trace("needbits="+y); return 0; }
                        x|=(buf&((1<<y)-1))<<havebits;
                        buf>>=y;
                        bufbits-=y;
                        needbits-=y;
        ...
        while(length!=0) {
                ...
                if(infmt&Defs.SF_ITPACKED!=0) {
                        ...
                        if (c_block==0) {
                                ...
                                incnt[0]=MMio._mm_read_I_UWORD(reader);
				...
				//trace("in 16-bit");
					...
					//trace("big endian");
                                        sl_buffer=MMio._mm_read_M_SWORDS(stodo,reader);
				}
                                else {
					//trace("little endian");
                                        sl_buffer=MMio._mm_read_I_SWORDS(stodo,reader);
				}
                        } else {
				//trace("in 8-bit");
				var ba=new ByteArray();
				reader.i.readBytes(ba,0,stodo);
				for (t in 0...stodo) {
					sl_buffer[t]=ba.readByte(); //signed
				}
                                for(t in 0 ... stodo) {
					sl_buffer[t]<<=8;
                                }
			...
			//trace("delta");
                        for(t in 0 ... stodo) {
                                sl_buffer[t] += sl_old;
				if (sl_buffer[t]<-32768) sl_buffer[t]+=65536;
				else if (sl_buffer[t]>32767) sl_buffer[t]-=65536;
                                sl_old = sl_buffer[t];
                        }
		}
			...
			//trace("flip signedness, in signed="+(infmt&Defs.SF_SIGNED!=0));
                        for(t in 0 ... stodo) {
                                //sl_buffer[t]^= 0x8000;
				sl_buffer[t]=(sl_buffer[t]&0xffff)-0x8000;	// will this work in all cases?
			}
		}
   ...
   static function DitherSamples(samplist:Array<SAMPLOAD>,type:Int):Int {
	var c2smp:SAMPLOAD;
	var maxsize:ULONG;
	var speed:ULONG;
	var si:Int;
...
Profiler.ENTER();
	...
	if (samplist[si]!=null) {	// while ...
		var s=samplist[si];
		var ns=samplist[si+1];
		if (ns!=null) { // the event will fire AFTER we return from this, so announce the NEXT sample
		   var n=ns.sample.samplename;
		   if (n==null || n=="") n="processing samples";
		   TrackerEventDispatcher.dispatchEvent(new TrackerLoadingEvent(0,n,si/sample_count));
		}
                ...
                if(s.sample.length!=0) {
                        if(s.sample.seekpos!=0)
                                MMio._mm_fseek(s.reader, s.sample.seekpos, MMio.SEEK_SET);
		...
		Profiler.LEAVE();
		return 0; // means "more to do here"
		//si++;
        }
        FreeSampleList(samplist);
Profiler.LEAVE();
   ...
   public static function SL_RegisterSample(s:SAMPLE,type:Int,reader:MREADER):SAMPLOAD {
	var samplist:Array<SAMPLOAD>;
	var news:SAMPLOAD;
